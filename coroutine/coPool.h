...
typedef void (*_callback)(void* args);
struct CoTask {
    _callback func;     //需要执行的任务
    void* args;         //函数参数(有时候给类的this使用)
    void* args2;        //函数参数2
    OUT UINT cid;       //函数执行时填充
    CoTask() : func(nullptr), args(nullptr), args2(nullptr), cid(0) {}
};
...
public:
    ...
    int terminate(UINT cid); //终结一个协程，但manager会重新创建的
    void scheduling();
};
